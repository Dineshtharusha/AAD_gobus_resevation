// ============= app.js — GoBus main page (connected to Spring Boot API) =============
// NOTE: API_BASE is defined in navigation.js (imported before this script)
...
searchForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // Update count
        document.querySelectorAll('.results-meta span')[2].textContent =
            `${allSchedules.length} bus${allSchedules.length !== 1 ? 'es' : ''} found`;
...
function buildScheduleCard(s) {
    ...
    const badge   = (s.busName || s.busNumber || 'BUS').substring(0, 2).toUpperCase();
    const badgeColor = ['teal', 'coral', 'blue', 'violet'][(s.busId || 0) % 4];
    ...
    <article class="journey-card" data-schedule-id="${s.id}" data-route="${s.source} -> ${s.destination}" data-dep="${depTime}" data-arr="${arrTime}">
        <div class="operator-badge ${badgeColor}">${badge}</div>
        <div class="journey-main">
            <div>
                <h3>${s.busName || s.busNumber} <span class="rating"><i data-lucide="armchair"></i> ${seats} seats left</span></h3>
...
function showNoResults(source, destination) {
    journeyList.innerHTML = `
        <div style="text-align:center;padding:48px 20px;color:#64748b;">
            ...
            <h3 style="color:#1e293b;margin-bottom:8px;">No buses found</h3>
            <p>No scheduled trips from <strong>${source}</strong> to <strong>${destination}</strong> on this date.</p>
            <p style="margin-top:8px;font-size:0.9em;">Try another date or check available routes below.</p>
        </div>`;
    if (window.lucide) lucide.createIcons();
    document.querySelectorAll('.results-meta span')[2].textContent = '0 buses found';
}
